...
Internet of Things, wherein individual components are composed via \emph{connectors}
...
\emph{Timed Reo} \cite{DBLP:conf/sefm/ArbabBBR04} is a real-time extension of the coordination language \emph{Reo}
\cite{DBLP:journals/mscs/Arbab04}, which can be used
...
For example, an operational semantics based on \emph{timed constraint automata} was proposed in 
\cite{DBLP:conf/sefm/ArbabBBR04}, where a variant of LTL was also proposed to describe the
...
e.g. a conformance testing framework on timed connectors has been proposed in \cite{DBLP:conf/tase/LiCWS15}.
A SAT-based bounded approach was adapted in \cite{DBLP:journals/scp/Kemper12} for model checking
connectors. Although these solutions are practical and impressive, there is one important
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=.45\textwidth]{./images/howto.png}
  \end{center}
...
\ref{sec:preliminaries} we illustrate some basic concepts, including the coordination
language Reo, Mealy machine, and active automata learning. Section \ref{sec:semantics} briefly presents an
operational semantics of Reo based on PMM, which is used in Section \ref{sec:activelearning} to show how to
...
\begin{itemize}
  ...
  \item [-] A \emph{FIFO1} channel is an asynchronous channel with a buffer cell. It accepts a
...
sink end on its corresponding node. A mixed node non-deterministically selects and takes a data item
...
Reo\cite{DBLP:conf/sefm/ArbabBBR04}, resource-sensitive timed Reo\cite{DBLP:conf/fmoods/MengA07}, hybrid Reo\cite{DBLP:conf/icfem/ChenSS14}, etc.
...
to pick up one from them deterministically.}.
...
Now we consider the reason that leads to existence of counter examples. Since
the suffix set $D$ is used to distinguish states, the existence of counter-examples
...
Moreover, with a CSP-style\cite{DBLP:books/ph/Hoare85} concurrency model, \texttt{Golang} shares many
...
simulation process could be reduced significantly. In this work, we use a multiway tree to buffer
these results.

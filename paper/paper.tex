...
In the learning algorithm, the most time-consuming operations are membership queries and
equivanlence queries. 
...
\section{Experiments}
Both \emph{Reo Coordination Models} and \emph{Adapted L* Algorithm} are implemented in Golang.
\todo{cite}
...
As shown in Table \ref{tabel:timecost}, time comsumption mainly comes from membership queries.
Since time is involved in our model, it's inevitably that simulation takes time to behave normally.
Even worse, since our models are treated as blackbox, there's little room for optimization. The only
way to reduce time cost is trying not to call the simulation process. Basic ideas are listed as
follows:
...
To reduce these repeated simulation, we use a cache to save all simulation results. 
\begin{table}[h]
  \renewcommand{\arraystretch}{1.3}
  ...
  \begin{tabular}{l||rrr}
    ...
    & FIFO & Alternator & Gate \\
    \hline\hline
    Before & 41.571 & 126.468 & 169.161 \\
    After & 41.571 & 126.468 & 169.161 \\
    \hline
  \end{tabular}
\end{table}

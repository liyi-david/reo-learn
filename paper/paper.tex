...
\maketitle
\begin{abstract}
  % reo coordination models
  Coordination languages provide a mean to formally specify the communication and
  reaction between different components. Such models play key roles in concurrent programming and
  large-scale distributed systems.
  % algorithms
  In this paper, an active learning framework is proposed to extract timed connectors from black-box
  systems.
  First of all, we introduce parameterized mealy machine as an operational semantics for
  channel-based coordination language Reo.
  Parameterized mealy machine serves as a bridge between Reo connectors and mealy machines. With
  production operator, complex connectors can be composed by basic channels and transformed into
  mealy machines. 
  Moreover, we adapt L*, a well-known learning algorithm, to timed connectors (in form of mealy
  machines). The new algorithm has shown its efficiency in multiple cases.
  % implementation
...
these results. A brief example of such trees can be found in Example \ref{example:tree}.

\begin{example}[Multiway-Tree Cache]
  \label{example:tree}
  Considering the FIFO channel presented in Example \ref{example:pmmfifo}. 
  \todo{draw a graph for multiway-tree cache}
\end{example}

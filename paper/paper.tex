...
In this paper we address this question by proposing an active learning framework as shown in Figure
...
Formally, an observation table (see Figure \ref{fig:hypo} as an example) is determined by a tuple $(S,D)$ where
...
labelled with input sequences. As shown in Figure \ref{fig:hypo}, rows are divided into two parts. The upper
...
\begin{figure}[ht]
  \begin{center}
    ...
    \end{displaymath}
  \end{center}
  \caption{A Closed Version of Figure \ref{fig:hypo}}
  \label{fig:hypo2}
\end{figure}
...
Figure \ref{fig:expfifosemantics} shows the learning result of this example where $n=2$ and
$\Sigma=\{a\}$.

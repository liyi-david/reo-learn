...
\begin{abstract}
  ...
  This framework has been implemented as a package in \texttt{Golang}.
\end{abstract}
...
operating systems. Consequently, manually extracting a proper model for an existing connector
...
\emph{Observation Tables}, proposed in \cite{DBLP:journals/iandc/Angluin87}, is used to construct
a hypothetical Mealy machine from the results of membership queries.

Generally, a Mealy machines is determined by its \emph{states} and \emph{transitions}.
As for blackboxes, input sequence $s\in\mathcal{I}^*$ can be used to denote its state 
after execution of $s$. Similarly, for all $a\in\mathcal{I}$, $sa$ indicates the successor
state of $s$ after execution of $a$.
However, under such notations, there are \emph{infinite} number of states where most are
reduplicate.

States are distinguished, usually, by their \emph{subsequent behavior}. Suppose $s_1\neq s_2\in
\mathcal{I}^*$, the corresponding of $s_1$ and $s_2$ are exactly the same iff $\forall d\in
...
\begin{definition}[$D$-Equivalence]
  ...
  the corresponding states of $s_1$ and $s_2$ are $D$-equivalent (denoted as $s_1\sim_D s_2$)
  iff $\forall d\in D, mq(s_1d) = mq(s_2d)$.
\end{definition}
\begin{definition}[Access Sequence]
  Given an input sequence $s$, we define the access sequence of $s$ as
  \[
  acc(s,D)=\min\{s'\in\mathcal{I}^*|s'\sim_D s\}
  \]
\end{definition}

Formally, an observation table is defined as a tuple $(S,D)$ where $S\subset\mathcal{I}^*$ indicates
the access sequences of states and $D\subset\mathcal{I}^*$ indicates the suffixes. Its
tabular form can be found in Figure \ref{fig:hypo}. In such tables, columns are labelled with
$d\in D$, and rows are labelled with input sequences. Rows are divided into two parts. The upper
part, labelled with $s\in S$, shows the states in current hypothesis (in form of access sequences).
The lower part, labelled with $sa$ (where $s\in S,a\in\mathcal{I}$) indicates all the transitions
from $S$. A cell with row label $s$ and column label $d$ is filled with $mq(sd)$.
...
\begin{itemize}
  ...
  \item[-] $s_0$ corresponds to the empty sequence $\varepsilon$
    ...
    \todo{define the empty sequence}
  \item[-] $\delta(s,a)=mq(sa)$
  \item[-] $\lambda(s,a)=acc(sa,obs.D)$
\end{itemize}
...
$unclosed$ set consists of access sequences that has no equivalent fellow in $S$. 
